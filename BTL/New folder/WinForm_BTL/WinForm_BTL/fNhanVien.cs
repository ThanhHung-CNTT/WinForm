...
namespace WinForm_BTL
{
    public partial class fNhanVien : Form
    {
        ...
        void NhanVienBinding()
        {
            ...
            cbMaChucVu.DataBindings.Add(new Binding("Text", dtgvNhanVien.DataSource, "TenCV", true, DataSourceUpdateMode.Never));
        ...
        void ThemDanhSachNhanVienVaoComboBox()
        {
            ...
            cbMaChucVu.DataSource = NhanVienDao.Instance.DanhSachNhanVien();
            cbMaChucVu.DisplayMember = "MaCV";
            cbMaChucVu.ValueMember = "TenCV";
        }

        private void btnThem_Click(object sender, EventArgs e)
        {
            using (var cmd = new SqlCommand("INSERT INTO NHANVIEN (MaNV, TenNV, GioiTinh, NgaySinh, DiaChi, CMND, Email, SDT, NgayVaoLam, TenCV) VALUES (@MaNV,@TenNV,@GioiTinh,@NgaySinh,@DiaChi,@CMND,@Email,@SDT,@NgayVaoLam,@TenCV)"))
            {
                ...
                cmd.Parameters.AddWithValue("@TenCV", cbMaChucVu.Text);
                connect.Open();
        ...
        {
            using (var cmd = new SqlCommand("UPDATE NHANVIEN set TenNV= @TenNV, GioiTinh= @GioiTinh, NgaySinh= @NgaySinh, DiaChi= @DiaChi, CMND= @CMND, Email= @Email,SDT= @SDT, NgayVaoLam= @NgayVaoLam, TenCV= @TenCV WHERE MaNV = @MaNV; "))
            {
                cmd.Connection = connect;
                cmd.Parameters.AddWithValue("@MaNV", Convert.ToInt32(cbMaNhanVien.Text));
                cmd.Parameters.AddWithValue("@TenNV", txtTenNhanVien.Text);
